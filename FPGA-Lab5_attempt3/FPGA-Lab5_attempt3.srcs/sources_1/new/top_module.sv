...
(
    ...
    typedef enum logic [1:0] { KEY_NONE, KEY_DETECTED, KEY_USED } KeyState_t;
    ...
    logic [3:0] read_data;
    logic blur_mode, dataReady;
    logic [7:0] scancode, prev_scancode;
    ...
    always @(negedge clk) begin
        ...
        if(prev_scancode == 8'hF0 && scancode == 8'h45 && dataReady) begin
            // key press was released, set flag to do a blur
            if (key_state == KEY_NONE) begin
                key_state <= KEY_DETECTED;
            end else begin
                key_state <= KEY_USED;
            end
        end else begin
            // something else happened, clear flag
            key_state <= KEY_NONE;
        end
     ...
     always_ff @(posedge clk) begin
        ...
        case (current_state)
            ...
            IDLE: begin
                ...
                pixelSum <= 0;
                trigger_copy <= 0;
                if (key_state == KEY_DETECTED) begin //(dataReady && scancode =<= 8'h45)
            ...
            PIXEL_WRITE: begin
                ...
                pixelSum <= pixelSum >> 3;
                next_state <= PRE_NEXT_PIXEL;
            end
            
            PRE_NEXT_PIXEL: begin
                if (o_valid) next_state <= NEXT_PIXEL_X;
                else next_state <= PRE_NEXT_PIXEL;
            end
            ...
            NEXT_PIXEL_Z: begin
                if(current_pixel_y >= HEIGHT) begin
                    next_state <= COPY_TO_OUTPUT;
                end else begin
                    next_state <= PIXEL_READ;
                end
            end
            
            COPY_TO_OUTPUT: begin
                if (o_valid) begin
                    trigger_copy <= 1;
                    next_state <= IDLE;
                end
